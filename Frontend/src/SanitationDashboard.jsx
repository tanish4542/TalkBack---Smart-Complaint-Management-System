...
import { FaSearch, FaReply, FaCheckCircle, FaBroom, FaSync } from 'react-icons/fa';
...
const SanitationDashboard = () => {
  ...
  const fetchComplaints = async () => {
    try {
      setIsLoading(true);
      const res = await API.get(`/api/sanitation?status=${filter}`);
      setComplaints(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      console.error('Error fetching sanitation complaints:', err);
      showError('Failed to fetch sanitation complaints.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async (complaint, newStatus) => {
    try {
      await API.put(`/api/sanitation/${complaint.id}/status`, {
        status: newStatus,
  ...
  const handleResponseSubmit = async (e) => {
    ...
    try {
      await API.post(`/api/sanitation/${activeComplaint.id}/response`, {
        response: responseText.trim(),
        resolvedBy: 'Sanitation Head',
    ...
    <AppShell>
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-3">
              ...
              Sanitation Head Workstation
            </h1>
            <p className="text-xs text-slate-500 mt-1">Review campus housekeeping and cleanliness tickets.</p>
          </div>
        ...
        {isLoading ? (
                      ...
                      <td className="px-6 py-4 max-w-md">
                        ...
                        {item.response && (
                          <div className="mt-1.5 p-2 bg-emerald-50 text-emerald-800 rounded-lg text-[11px]">
                            <strong>Head Response:</strong> {item.response}
                          </div>
                        )}
                      </td>
          ...
          <EmptyState title="No Sanitation Tickets" description="Queue is empty for this criteria." />
        )}
      </div>
