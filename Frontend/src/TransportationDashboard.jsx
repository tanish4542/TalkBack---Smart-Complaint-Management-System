...
import { FaSearch, FaReply, FaCheckCircle, FaBus, FaSync } from 'react-icons/fa';
...
const TransportationDashboard = () => {
  ...
  const fetchComplaints = async () => {
    try {
      setIsLoading(true);
      const res = await API.get(`/api/transportation?status=${filter}`);
      setComplaints(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      console.error('Error fetching transport complaints:', err);
      showError('Failed to fetch transport complaints.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async (complaint, newStatus) => {
    try {
      await API.put(`/api/transportation/${complaint.id}/status`, {
        status: newStatus,
  ...
  const handleResponseSubmit = async (e) => {
    ...
    try {
      await API.post(`/api/transportation/${activeComplaint.id}/response`, {
      ...
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Review campus bus, driver, and scheduling tickets.</p>
          </div>
        ...
        {isLoading ? (
                      ...
                      <td className="px-6 py-4 max-w-md">
                        ...
                        {item.response && (
                          <div className="mt-1.5 p-2 bg-emerald-50 text-emerald-800 rounded-lg text-[11px]">
                            <strong>Manager Response:</strong> {item.response}
                          </div>
                        )}
                      </td>
          ...
          <EmptyState title="No Transport Tickets" description="Queue is empty for this criteria." />
        )}
      </div>
        ...
        {activeComplaint && (
          <form onSubmit={handleResponseSubmit} className="space-y-4">
            ...
            <textarea
              rows={4}
              required
              placeholder="Enter transport manager response..."
              value={responseText}
