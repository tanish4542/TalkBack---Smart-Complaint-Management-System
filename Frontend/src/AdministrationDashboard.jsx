...
import { FaSearch, FaReply, FaCheckCircle, FaUniversity, FaSync } from 'react-icons/fa';
...
const AdministrationDashboard = () => {
  ...
  const fetchComplaints = async () => {
    try {
      setIsLoading(true);
      const res = await API.get(`/api/administration?status=${filter}`);
      setComplaints(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      console.error('Error fetching admin complaints:', err);
      showError('Failed to fetch administration complaints.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async (complaint, newStatus) => {
    try {
      await API.put(`/api/administration/${complaint.id}/status`, {
        status: newStatus,
      ...
      if (err.response?.status === 409) {
        showError('Concurrency Conflict: Ticket updated by another admin. Refreshing...');
        fetchComplaints();
      } else {
  ...
  const handleResponseSubmit = async (e) => {
    ...
    try {
      await API.post(`/api/administration/${activeComplaint.id}/response`, {
      ...
      if (err.response?.status === 409) {
        showError('Concurrency Conflict: Refreshed latest state.');
        setActiveComplaint(null);
        fetchComplaints();
      } else {
      ...
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Review fee, document, and administrative tickets.</p>
          </div>
        ...
        {isLoading ? (
          ...
          <EmptyState title="No Complaints" description="No administrative tickets match criteria." />
        )}
      </div>
