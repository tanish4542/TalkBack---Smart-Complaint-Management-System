...
import { FaBed, FaHistory, FaLock } from 'react-icons/fa';
...
const HostelComplaint = () => {
  ...
  const fetchHistory = async () => {
    try {
      const res = await API.get('/api/hostel/history');
  ...
  const handleSubmit = async (e) => {
    ...
    if (!description || !description.trim()) {
      showError('Please enter a description for your hostel complaint.');
      return;
    }

    setIsSubmitting(true);
    try {
      const res = await API.post('/api/hostel/submit', {
      ...
      if (res.status === 200) {
        if (isAnonymous && res.data?.trackingToken) {
          ...
          showSuccess('Hostel complaint submitted successfully!');
        }
        setDescription('');
        setHostelBlock('');
        setRoomNumber('');
      ...
      showError(err.response?.data?.error || 'Failed to submit hostel complaint.');
    } finally {
      ...
      <div className="space-y-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-3">
              ...
              Hostel Facilities Portal
            </h1>
            <p className="text-xs text-slate-500 mt-1">
              Report room maintenance, plumbing, electrical, or hostel facility issues.
            </p>
          </div>
        ...
        <Card className="shadow-lg">
          <CardHeader className="bg-slate-50/50">
            <CardTitle>File Hostel Maintenance Ticket</CardTitle>
            <CardDescription>Enter block and room details for hostel warden action</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <textarea
                  rows={4}
                  required
                  placeholder="Describe electrical, plumbing, or facility maintenance issue..."
                  value={description}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold uppercase text-slate-700 mb-1.5">Hostel Block</label>
                  <select
                    value={hostelBlock}
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase text-slate-700 mb-1.5">Room Number</label>
                  <input
                    type="text"
                    placeholder="e.g. 302"
                    value={roomNumber}
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-start gap-3">
                ...
                />
                <label htmlFor="anon-hostel" className="text-xs font-bold text-slate-900 cursor-pointer flex items-center gap-1.5">
                  <FaLock className="text-amber-500" /> Submit Anonymously
                </label>
              </div>

              <Button type="submit" variant="primary" size="lg" className="w-full" isLoading={isSubmitting}>
                Submit Hostel Ticket
              </Button>
            </form>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><FaHistory /> Hostel Maintenance History</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {history.length > 0 ? (
                  ...
                  <div key={c.id} className="p-6">
                    ...
                    {c.response && (
                      <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200 text-xs mt-2">
                        <span className="font-bold text-emerald-900 block">Warden Response ({c.resolved_by || 'Hostel Warden'}):</span>
              ...
              <div className="p-8">
                <EmptyState title="No Hostel History" description="No hostel maintenance tickets recorded." />
              </div>
            )}
          </CardContent>
        </Card>
